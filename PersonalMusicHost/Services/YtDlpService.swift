...
enum YtDlpError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notInstalled:
            return "yt-dlp chưa được cài đặt tại /opt/homebrew/bin/yt-dlp"
...
final class YtDlpService {
    ...
    /// Tải file audio trực tiếp (.m4a) về đĩa thông qua yt-dlp (cực nhanh, chuẩn format)
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            do {
                ...
                if process.terminationStatus == 0 {
                    // Tìm file vừa tải
                    let possibleExtensions = ["m4a", "webm", "opus", "mp3"]
                    var foundURL: URL?
                    ...
                    let errMsg = String(data: errData, encoding: .utf8) ?? "Unknown yt-dlp error"
                    continuation.resume(throwing: YtDlpError.executionFailed(errMsg))
                }
            } catch {
    ...
    /// Lấy metadata trực tiếp từ link bất kỳ thông qua yt-dlp --dump-json
            ...
            do {
                ...
                if process.terminationStatus == 0 {
                    ...
                    if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                        ...
                        continuation.resume(throwing: YtDlpError.executionFailed("Không thể parse JSON từ yt-dlp"))
                    }
                } else {
                    let errData = errPipe.fileHandleForReading.readDataToEndOfFile()
                    let errMsg = String(data: errData, encoding: .utf8) ?? "Unknown yt-dlp error"
